...
- **Known exception, accepted rather than missed:** `accept-invitation.ts` and `reject-invitation.ts` pass Better Auth's own message through as `BadRequestError(error.message)`, so an un-audited upstream string reaches the user. These sites were inspected and kept; auditing the copy is tracked separately.
