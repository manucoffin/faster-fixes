...
Faster Fixes, the tool we build, is that layer: a widget you add to your staging build so stakeholders comment on the page itself, and every report reaches you with its element, URL, viewport and browser already attached. Installing it takes one script tag, or a component in a React app, and a project ID - see the [quickstart](/docs/getting-started/quickstart).
