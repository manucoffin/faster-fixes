...
Components that read `isVisible`, `feedbackItems` or `showPins` re-render when the value changes. Before the widget mounts on the client, the hook returns `isVisible: false`, `feedbackItems: []` and `showPins: true`, and its methods do nothing.
...
| `startAnnotation` | `() => void`     | Show the widget and enter annotation mode (user can click elements) |
...
Start annotation from your own UI. The floating button stays on the page:
