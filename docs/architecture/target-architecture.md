...
- **Inngest is present.** `src/server/inngest/` holds the client and the event definitions (`events.ts`, one Zod-typed `eventType` per event, shared by the functions that trigger on them and the services that send them) and nothing else: each durable function is a `*.inngest.ts` service in the `_services/` bucket of the domain it drives, and `app/api/inngest/route.ts` registers the full list by deep path as route-tier composition. So the Inngest boundary row of the mapping table and the `*.inngest.ts` service convention are live here rather than `[if present]`.
